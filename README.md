...
| **GPIO 19, 20** | USB-JTAG (usable if USB-OTG is not needed; GPIO 20 is used as MAX6675 MISO in this project) |
| **GPIO 43, 44** | UART0 TX/RX — used by the serial console / REPL |
| **GPIO 48** | Directly drives the on-board RGB LED |
| **GPIO 33–37** | **N8R8 only** — reserved by the Octal-SPIRAM (PSRAM) interface; free to use on the N8 variant (no PSRAM) |
| **GPIO 9–11** | Default HSPI pins — MicroPython maps these to `SPI(1)` internally; reassigning them to other functions may conflict |
...
[ESP32-S3-DevKitC-1 schematic](https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32s3/esp32-s3-devkitc-1/index.html).
